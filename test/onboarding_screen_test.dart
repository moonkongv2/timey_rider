...
void main() {
  testWidgets('Korean onboarding first card shows localized copy', (
    ...
    expect(find.text('시간은 아직 눈에 잘 보이지 않아요'), findsOneWidget);
    expect(find.textContaining('매일의 루틴이 작은 씨름처럼 느껴질 때가 있죠.'), findsOneWidget);
    expect(find.text('괜찮아, 같이 시작해요'), findsOneWidget);
  });
  ...
  testWidgets('Next button advances to the second onboarding card', (
    ...
    expect(find.text('오늘의 루틴을 작은 라이딩으로 바꿔요'), findsOneWidget);
  });
